...
packages <- c("ggrepel", "ggplot2",  "data.table", "tm", "wordcloud2",
              "tidytext", "dplyr", 'tidyverse', 'readxl', 'udpipe',
              'writexl', 'openxlsx', 'rlang', 'lsa', 'shiny')
 
InstallOrLoadPack(packages)

Wordcloud2a <- function (data, size = 1, minSize = 0, gridSize = 0, 
                         fontFamily = "Segoe UI", fontWeight = "bold", 
                         color = "random-dark", backgroundColor = "white", 
                         minRotation = -pi/4, maxRotation = pi/4, 
                         shuffle = TRUE, rotateRatio = 0.4, 
...
GetListOfStopwords <- function() {
  ...
  stop_words_expanded <- c('и', 'й', 'г', 'единаяроссия', 'единый', 'время', 
     'территория', 'димитровграда', 'димитровград', 'чебоксар', 'ядринский', 
     'житель', 'компания', 'министр', 'дом', 'общественный',
     'программа', 'мероприятие', 'условие', 'ситуация', 
     'гражданин', 'группа', 'организация','система', 'оао', 'ооо', 
      'пао', 'зао', 'центр',  'руководитель', 'конкурс', 'решить', 
     'говорить', 'состав', 'уровень', 'адрес', 'дело', 'заместитель', 
     'просто', 'данный', 'сайт', 'позолять', 'директор', 'полный', 'час', 
     'неделя', 'рука', 'ядрин',  'погода', 'выбрать',
     'средство', 'принять', 'чебоксарский', 'пункт', 'получать', 'второй', 
      'количество', 'найти', 'pgrunews', 'хороший', 'занятие', 'ныжный', 
     'номер', 'отлично', 'лс',
     ...
     "дождь", "м", "кубок", "фонд", "поздравляе", "мужчина", 
     "женщина", "проводить", "пробный", "узнать", "материал", "статья",
      "отдел", "денежный", "полицейский", "сторона", "допустить", 
     "вещество", "соблюдать", "вызов", "мобильный", "знакомый", "родной", 
     "прохождение", "плата", "выплата", "качество", "официальный", 
     "комфортный", "оборудование", "обязанность",  "добавить", 
     "предоставление", "размер", "поставить", "касаться", "подготовить", 
     "известный","госсоветерт", "госсоветерт", "ч", "отношение", "завтра",
     
     'ульяновскаяобласть', 'ульяновск', 'ульяновский', 'саранск', 'саранска', 
     'мордовие', 'рм', 'рма', 'мордовия', 'мордовский', 'заец', 'idюрий', 
     'главамарийэл', 'марий', 'эл', 'марийэл', 'эть', 'васил', 'чурин', 
     'кировский', 'кировскаяобласть', 'вятский', 'мельниченко', 'месяц', 
     'оренбургнуть', 'объясняемрф', 'провести', 'инвестор', 'вести', 
      'реализация', 'башкортостанный', 'радий', 'подписать', 'проект', 
     'пермский', 'пермскийкрай', 'край', 'прикамья', 'краевой', 'задача', 
     'важно', 'оренбуржец', 'оренбург', 'новость', 'подчеркнуть', 'оренбуржье', 
     'оренбургский', 'оренбургскаяобласть', 'поддержка', 'часть', 'км', 
     'валерийрадаеть', 'олегнуть', 'должен', 'около', 'рассказать', 'глава', 
     'губернатор', 'развитие', 'январь', 'февраль', 'март', 'апрель', 'май', 
     'июнь', 'июль', 'август', 'сентябрь', 'октябрь', 'ноябрь', 'декабрь', 
     'город', 'ecom', 'казань', 'подробность', 'подробный', 'радия', 'процент', 
     'уф', 'часть', 'вопрос', 'делать', 'сделать', 'благодаря', 'участие', 
     'пройти', 'идти', 'создать', 'создавать', 'дать',  'рамка', 'место', 
     'получить', 'удмуртия', 'радай', 'юлие', 'пензенский', 'пенза', 
     'пензенскаяобласть', 'новый', 'лучший', 'самый', 'работа', 'рабочий',
     'работать', 'региональный', 'нижегородскаяобладать', 'clubнижегородский',
     'нижегородскаяобласть', 'нижегородский', 'нижний', 'новгород', 'чувашия', 
     'чувашие', 'обть', "бaшҡортостать", "бaшҡортостан", 'командахабиров', 
     'рб', 'миллиард', 'башкирия', 'башкортостан', 
     'башкортостана', 'мый', 'аный',  
     'мухаметшина', 'мухаметшин', 'реть', 'рф', 'день', 'отметить', 'число', 
     'миллион', 'ход', 'президент','страна', 'тысяча', 'рубль', 'доллар', 
     'район', 'итог', 'татарстан', 'татарстать', 'российский', 'ма', 
     'область', 'республика', 'саратовский', 'татарстан', 'татарстана', 
     'самарский', 'экономический', 'экономика', 'регион', 'год',
      ...
      'азар', 'стать', 'rn«', 'твой',
      'сих', 'ком', 'свой', 'слишком', 'нами', 'всему', 'будь', 
      'саму', 'чаще',  'ваше', 'наш', 'затем', 'еще', 'наши', 
      'ту', 'каждый',
     'мочь', 'весь', 'этим', 'наша', 'своих', 'оба', 'который', 'зато', 
     'те', 'вся', 'ваш', 'такая', 'теми', 'ею', 'нередко',
      'также', 'чему', 'собой',  'нем', 'вами', 'ими', 'откуда', 
      'такие', 'тому', 'та', 'очень', 'нему',  'д',
     'алло', 'оно', 'кому', 'тобой', 'таки', 'мой', 'нею', 'ваши', 
      'ваша', 'кем', 'мои', 'однако', 'сразу', 'свое', 'ними', 'всё', 
      'неё', 'тех', 'хотя', 'всем', 'тобою', 
     'тебе', 'одной', 'другие', 'буду', 'моё', 'своей', 'такое', 
     'всею', 'будут', 'своего', 'кого', 'свои', 'мог', 'нам', 
    'особенно', 'её', 'наше', 'кроме', 'вообще', 'вон', 'мною', 'никто', 
    'это', 'изза', 'именно', 'поэтому', 'будьт', 
     'являться', 'чувашский', 'тыса', 'смочь', 'ваший', 
      'гльба', 'ать', 'уть',  'ивать', 'ольги', 'пенз', 'ер',
        'иметь', 'олегнуть', 'сг', 'например', 
      'сообщить', 'сообщать', 'среди', 'нть', 'пер', 
        'зспермь', 'края', 'ради', 
      'назвать', 'важный', 'ик', 'ульяновсок', 
      'ульяновска', 'russia', 'reg',
      'видео', 'russianpolice', 'mvd', 'police', 'ульяновскаяобласть', 
...
server <- function(input, output, session) {
  ...
  GetRakeKeywords <- function(file) {
    req(file)
    showNotification(label_calculation_begining, duration = 
                     time_of_notification_duration)
    ...
    gsub("[«№\U{1F600}-\U{1F64F}\U{1F300}-\U{1F5FF}\U{1F680}-\U{1F6FF}
      \U{1F1E0}-\U{1F1FF}\U{2500}-\U{2BEF}\U{2702}-\U{27B0}
      \U{24C2}-\U{1F251}\U{1f926}-\U{1f937}
      \U{10000}-\U{10ffff}\u{2640}-\u{2642}\u{2600}-\u{2B55}
      \u{200d}\u{23cf}\u{23e9}
      \u{231a}\u{fe0f}\u{3030}\U{00B0}\U{20BD}]",
         "", corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- gsub("\\b\\S*(http|vk)\\S*\\b", "", 
                        corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- gsub("движениепервых", 
                                             "#движение первых#", 
                       corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- gsub("навигаторыдетство", 
                                             "#навигаторы детство#", 
                       corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- gsub("новостипервых", 
                                             "#новости первых#", 
                       corp_city_df[["1"]][["content"]], perl = TRUE)
    ...
    corp_city_df[["1"]][["content"]] <- gsub("большаяучительскаянеделя", 
                       ...
                       corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- gsub("годпедагоганаставник", 
                                             "#год педагога наставник#",
                          corp_city_df[["1"]][["content"]], perl = TRUE)
    ...
    x <- udpipe_annotate(gsd_model, x = corp_city_df[["1"]][["content"]], 
                         parser = "none")
    x <- as.data.frame(x)   
    tmp <- x$lemma 
    ...
    x$lemma <- tmp
    keywords_rake_test <- function(x, term, group, relevant = 
                                   rep(TRUE, nrow(x)), 
      ...
      if (length(group) > 1) {
        ...
        x <- data.table(group = x[[group]], word = x[[term]],
                        .relevant = relevant)
      }
      ...
      x <- x[, `:=`(keyword, paste(word, collapse = sep)),
             by = list(keyword_id)]
      x <- x[, `:=`(degree, .N - 1L), by = list(keyword_id)]
      word_score <- list()
      word_score$degree <- x[, list(degree = sum(degree)), 
                             by = list(word)]
      ...
      keywords <- x[, list(freq = length(unique(keyword_id))), 
                    by = list(keyword, word)]
      keywords <- merge(keywords, 
                    word_score[, c("word", "rake_word_score"), with = FALSE],
      ...
      keywords <- data.table::setcolorder(keywords, 
                          neworder = c("keyword", "ngram", "freq", "rake"))
    ...
    keywords_rake_df <- keywords_rake_test(x, term = "lemma", 
                             group = c("sentence_id"),
  ...
  GetPreprocessedTextsWordList <- function(file) {   
    req(file)
    showNotification(label_calculation_begining, 
                     duration = time_of_notification_duration)
    input_data <- as.data.frame(read_excel(file$datapath, 
                                           col_names = FALSE)) 
    corp_city_df <- 
      CleanCorpusFrequency(VCorpus(VectorSource(input_data)))
    corp_city_df[["1"]][["content"]] <- gsub("[\U{1F600}-\U{1F64F}
      \U{1F300}-\U{1F5FF}
      \U{1F680}-\U{1F6FF}\U{1F1E0}-\U{1F1FF}
    \U{2500}-\U{2BEF}\U{2702}-\U{27B0}
      \U{24C2}-\U{1F251}\U{1f926}-\U{1f937}\U{10000}-\U{10ffff}
    \u{2640}-\u{2642}\u{2600}-\u{2B55}
      \u{200d}\u{23cf}\u{23e9}\u{231a}\u{fe0f}\u{3030}
    \U{00B0}\U{20BD}]", "", 
                           corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- gsub("\\b\\S*(http|vk)\\S*\\b", "",
                           corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- 
        gsub("движениепервых", "движение первых", 
               corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- 
      gsub("навигаторыдетство", "навигаторы детство", 
               corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- 
      gsub("новостипервых", "новости первых", 
               corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- gsub("фотоомск", "фото омск", 
               corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- 
      gsub("большаяучительскаянеделя", "большая учительская неделя", 
               corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- 
      gsub("годпедагоганаставник", "год педагога наставник", 
               corp_city_df[["1"]][["content"]], perl = TRUE)
    corp_city_df[["1"]][["content"]] <- 
      gsub("государственныйсоветреспублики", 
               "государственный совет республики ", 
               corp_city_df[["1"]][["content"]], perl = TRUE)
  ...
  AnalyzeAndRenderRake <-  function(file_input, id_plot_output, 
                        id_table_output, id_wordcloud_output) { 
    keywords_rake_df <- GetRakeKeywords(file_input)
    keywords_rake_df_for_output <-
      keywords_rake_df[c("keyword", "freq", "rake")]
    name_of_input_file <- file_input$name
    output[[id_plot_output]] <- renderPlot({
      diagram_for_output <- ggplot(keywords_rake_df_for_output[1:10, ],
        aes(x = reorder(keyword, rake), y = rake)) +
        geom_bar(stat = "identity") +
        coord_flip() +
        labs(title = "Слова с наибольшим индексом RAKE", 
             x = "Слова", y = "Индекс RAKE") +
    ...
    output[[id_table_output]] <- renderTable({
      colnames(keywords_rake_df_for_output) <- 
        c("Ключевые слова", "Частота встречаемости", "RAKE")
    ...
    keywords_rake_df_for_output_wordcloud <- 
      keywords_rake_df_for_output[c("keyword", "freq")]
    # Сортировка по столбцу freq по убыванию для облака слов
    keywords_rake_df_for_output_wordcloud <- 
      keywords_rake_df_for_output_wordcloud[
        order(keywords_rake_df_for_output_wordcloud$freq, 
              decreasing = TRUE),]
    output[[id_wordcloud_output]] <- renderWordcloud2({
      wordcloud_rake_for_output <- 
          head(keywords_rake_df_for_output_wordcloud,
               amount_of_words_in_wordcloud)
    ...
    showNotification(label_calculation_end, duration = 
                     time_of_notification_duration)
    return(keywords_rake_df)
  }
  
  AnalyzeAndRenderFrequency <- function(file_input, id_plot_output, 
                          id_table_output, id_wordcloud_output) {   
    preprocessed_texts_word_list <- 
      GetPreprocessedTextsWordList(file_input)
    d <- as.data.frame(sort(table(preprocessed_texts_word_list), 
                            decreasing = TRUE))
    ...
    output[[id_plot_output]]  <- renderPlot({
      barplot_tf <- ggplot(d_word_tf[1:10, ], 
        aes(x = reorder(word, tf), y = tf)) +
        geom_bar(stat = "identity") +
        coord_flip() +
        labs(title = "Ключевые слова", x = "Слова", 
             y = "Term Frequency") +
        theme_gray(base_size = 26)
      name_of_input_file <- file_input$name
    ...
    output[[id_wordcloud_output]] <- renderWordcloud2({
      wordcloud_tf_for_output <- head(d_word_tf, 
                                      amount_of_words_in_wordcloud)
      ...
      Wordcloud2a(head(d_word_tf, amount_of_words_in_wordcloud), 
                  size = 0.45)
    })
    showNotification(label_calculation_end, duration = 
                     time_of_notification_duration)
  ...
  ObserveEventCompareFilesBtnFrequency <- function(){
    d_all <- Filter(Negate(is.null), 
                    list(files_preprocessed_data_frequency[["df_1"]], 
    ...
    if (length(d_all) <= 1) {
      showNotification(
        "Для анализа должно быть обработано не менее двух файлов
    ...
    {  
      ...
      {
        names(d_all[[i]]) <- c("word", paste("freq", i, sep = ""),
                               paste("tf", i, sep = ""))
      }
      res <- d_all[[1]]
      ...
      for (i in 1:amount_of_processed_files)
      {
        col_names_word_freq_tf <- c(col_names_word_freq_tf,  
            paste("freq", i, sep = ""), paste("tf", i, sep = ""))
        col_names_word_freq <- c(col_names_word_freq, 
            paste("freq", i, sep = ""))
        col_names_freq <- c(col_names_freq, 
            paste("freq", i, sep = ""))
      }
      ...
      tdm_df <- tdm_df %>% mutate(num_of_occurrences = 
                                  rowSums(select(tdm_df,
                                          all_of(col_names_freq)) != 0))
      tdm_df <- tdm_df %>% mutate(idf = 
                                  log((amount_of_processed_files + 1) / 
                                      (1 + num_of_occurrences) + 1))
      tdm_df_with_dynamism <- tdm_df
      tdm_df_with_dynamism$freq_all <- tdm_df_with_dynamism[['freq1']]
      for (i in 2:amount_of_processed_files)
      {
        tdm_df_with_dynamism$freq_all <- 
          tdm_df_with_dynamism$freq_all +
            tdm_df_with_dynamism[[paste("freq", i, sep = "")]]
      }
      tdm_df_with_dynamism$dynamism <- tdm_df_with_dynamism[[
        paste("freq", amount_of_processed_files, sep = "")]] -
      ...
      tdm_df_with_dynamism$freq_all_normalized <- 
        (tdm_df_with_dynamism$freq_all) / 
          ifelse(max(tdm_df_with_dynamism$freq_all) != 0, 
                 max(tdm_df_with_dynamism$freq_all), 1)  
      tdm_df_with_dynamism$dynamism_normalized <- (
        tdm_df_with_dynamism$dynamism) / 
          ifelse(max(tdm_df_with_dynamism$dynamism) != 0,
                 max(tdm_df_with_dynamism$dynamism), 1)  
      tdm_df_with_dynamism$freq_all_and_dynamism_normalized <- 
          tdm_df_with_dynamism$dynamism_normalized + 
            tdm_df_with_dynamism$freq_all_normalized
      tdm_df_with_dynamism <- tdm_df_with_dynamism[
        order(tdm_df_with_dynamism$freq_all_and_dynamism_normalized, 
              decreasing = TRUE),] 
      ...
      output$dynamicPlotLimited <- renderPlot({
        ...
        tdm_df_with_dynamism_limited <- 
          tdm_df_with_dynamism[1:amount_of_words_in_plot, ]
        tdm_df_with_dynamism_limited$dynamism_shifted_for_30 <- 
         tdm_df_with_dynamism_limited$dynamism - 
           min(tdm_df_with_dynamism_limited$dynamism)
        tdm_df_with_dynamism_limited$dynamism_normalized_for_30 <- 
         tdm_df_with_dynamism_limited$dynamism_shifted_for_30 /
           ifelse(max(
             tdm_df_with_dynamism_limited$dynamism_shifted_for_30) != 0,
             max(tdm_df_with_dynamism_limited$dynamism_shifted_for_30), 1)
        tdm_df_with_dynamism_limited$freq_all_shifted_for_30 <- 
          tdm_df_with_dynamism_limited$freq_all - 
            min(tdm_df_with_dynamism_limited$freq_all)
        tdm_df_with_dynamism_limited$freq_all_normalized_for_30 <- 
          (tdm_df_with_dynamism_limited$freq_all_shifted_for_30) /
            ifelse(max(tdm_df_with_dynamism_limited$freq_all_shifted_for_30) 
                   != 0,
                   max(tdm_df_with_dynamism_limited$freq_all_shifted_for_30), 
                   1)
        dynamicPlotLimited <- 
          ggplot(tdm_df_with_dynamism_limited[1:amount_of_words_in_plot, ], 
  ...
  ObserveEventCompareFilesBtnRake <- function() {
    d_all <- Filter(Negate(is.null), 
                    list(files_preprocessed_data_rake[["df_1"]],
    ...
    cos.mat <- NULL
    if (length(d_all) <= 1) {
      showNotification("Для анализа должно быть обработано 
                       не менее двух файлов с помощью одного метода.",
                       duration = time_of_notification_duration)
    }
    else
    {
      ...
      for (i in 1:amount_of_processed_files)
      {
        col_names_keyword_rake <- c(col_names_keyword_rake, 
                                    paste("rake", i, sep = ""))
      ...
      for (i in 1:amount_of_processed_files)
      {
        period_names <- c(period_names, paste0("Период ",
                                               i, sep = ""))
      }
      ...
      rake_df_with_dynamism$dynamism <- 
        rake_df_with_dynamism[[paste("rake", amount_of_processed_files,
                                     sep = "")]]
      ...
      rake_df_with_dynamism$rake_all_normalized <- 
       (rake_df_with_dynamism$rake_all) /
        ifelse(max(rake_df_with_dynamism$rake_all) != 0,
               max(rake_df_with_dynamism$rake_all), 1)

      value_for_norm_of_dynamic <- 
        ifelse(min(rake_df_with_dynamism$dynamism) < 0,
           -min(rake_df_with_dynamism$dynamism), 0)
      rake_df_with_dynamism$dynamism_normalized <- 
        (rake_df_with_dynamism$dynamism +
        value_for_norm_of_dynamic) /
        ifelse(max(rake_df_with_dynamism$dynamism + 
        value_for_norm_of_dynamic) != 0,
        max(rake_df_with_dynamism$dynamism + value_for_norm_of_dynamic),
               1)
      rake_df_with_dynamism$sum_of_rake_all_norm_and_dyn_norm <-
        rake_df_with_dynamism$dynamism_normalized + 
        rake_df_with_dynamism$rake_all_normalized
     
      rake_df_with_dynamism <- rake_df_with_dynamism[
        order(rake_df_with_dynamism$sum_of_rake_all_norm_and_dyn_norm,
              decreasing = TRUE),]
      output$compareFilesTable <- renderTable({
      ...
      output$dynamicPlotLimited <- renderPlot({
        amount_of_words_in_plot <- 10
        rake_df_with_dynamism_limited <- 
          rake_df_with_dynamism[1:amount_of_words_in_plot, ]
        rake_df_with_dynamism_limited$dynamism_shifted_for_30 <- 
          rake_df_with_dynamism_limited$dynamism -
            min(rake_df_with_dynamism_limited$dynamism)
        rake_df_with_dynamism_limited$dynamism_normalized_for_30 <- 
          rake_df_with_dynamism_limited$dynamism_shifted_for_30 /
            ifelse(max(rake_df_with_dynamism_limited$dynamism_shifted_for_30) 
                   != 0,
                 max(rake_df_with_dynamism_limited$dynamism_shifted_for_30), 
                   1)
        rake_df_with_dynamism_limited$rake_all_shifted_for_30 <- 
          rake_df_with_dynamism_limited$rake_all - 
            min(rake_df_with_dynamism_limited$rake_all)
        rake_df_with_dynamism_limited$rake_all_normalized_for_30 <- 
          (rake_df_with_dynamism_limited$rake_all_shifted_for_30) /
            ifelse(max(rake_df_with_dynamism_limited$rake_all_shifted_for_30) 
                   != 0,
                 max(rake_df_with_dynamism_limited$rake_all_shifted_for_30), 
                   1)
        plot_limited <- ggplot(rake_df_with_dynamism_limited,
         aes(x = dynamism_normalized_for_30, y = rake_all_normalized_for_30,
             label = keyword)) +
          geom_point() +
          geom_text_repel(max.overlaps = 10, size = 7) +
          labs(x = "Динамика", y = "Значимость", 
        ...
        if (input$radio == 2) {
          if (!is.null(wordcloud_data_rake_1()) && 
            ...
            print(wordcloud(wordcloud_data_rake_1()[["keyword"]], 
                    wordcloud_data_rake_1()[["freq"]], min.freq = 0))
            dev.off()
          ...
          if (!is.null(wordcloud_data_tf_1()) && 
            ...
            print(wordcloud(wordcloud_data_tf_1()[["word"]], 
                            wordcloud_data_tf_1()[["tf"]], , min.freq = 0))
            dev.off()
      ...
      tryCatch({
        if (input$radio == 2) {
          if (!is.null(wordcloud_data_rake_2()) && 
            ...
            f3_rake <- "table_rake.csv"
            png(f1_rake, width = 800, height = 600)
            print(wordcloud(wordcloud_data_rake_2()[["keyword"]],
                            wordcloud_data_rake_2()[["freq"]], min.freq = 0))
            dev.off()
            png(f2_rake, width = 800, height = 600)
          ...
          if (!is.null(wordcloud_data_tf_2()) && 
            ...
            f3_tf <- "table_tf.csv"
            png(f1_tf, width = 800, height = 600)
            print(wordcloud(wordcloud_data_tf_2()[["word"]],
                            wordcloud_data_tf_2()[["tf"]], min.freq = 0))
            dev.off()
            png(f2_tf, width = 800, height = 600)
            print(barplot_tf_2())
            dev.off()
      ...
      tryCatch({
        if (input$radio == 2) {
          ...
          if (!is.null(wordcloud_data_rake_3()) && 
            ...
            f3_rake <- "table_rake.csv"
            png(f1_rake, width = 800, height = 600)
            print(wordcloud(wordcloud_data_rake_3()[["keyword"]],
                            wordcloud_data_rake_3()[["freq"]], min.freq = 0))
            dev.off()
            png(f2_rake, width = 800, height = 600)
          ...
          if (!is.null(wordcloud_data_tf_3()) && 
            ...
            f3_tf <- "table_tf.csv"
            png(f1_tf, width = 800, height = 600)
            print(wordcloud(wordcloud_data_tf_3()[["word"]],
                            wordcloud_data_tf_3()[["tf"]], min.freq = 0))
            dev.off()
            png(f2_tf, width = 800, height = 600)
            print(barplot_tf_3())
            dev.off()
      ...
      tryCatch({
        if (input$radio == 2) {
          if (!is.null(wordcloud_data_rake_4()) && 
            ...
            f3_rake <- "table_rake.csv"
            png(f1_rake, width = 800, height = 600)
            print(wordcloud(wordcloud_data_rake_4()[["keyword"]], 
                            wordcloud_data_rake_4()[["freq"]], min.freq = 0))
            dev.off()
            png(f2_rake, width = 800, height = 600)
          ...
          if (!is.null(wordcloud_data_tf_4()) && 
            ...
            f3_tf <- "table_tf.csv"
            png(f1_tf, width = 800, height = 600)
            print(wordcloud(wordcloud_data_tf_4()[["word"]], 
                            wordcloud_data_tf_4()[["tf"]], min.freq = 0))
            dev.off()
            png(f2_tf, width = 800, height = 600)
            print(barplot_tf_4())
            dev.off()
      ...
      tryCatch({
        if (input$radio == 2) {
          if (!is.null(wordcloud_data_rake_5()) && 
            ...
            f3_rake <- "table_rake.csv"
            png(f1_rake, width = 800, height = 600)
            print(wordcloud(wordcloud_data_rake_5()[["keyword"]],
                            wordcloud_data_rake_5()[["freq"]], min.freq = 0))
            dev.off()
            png(f2_rake, width = 800, height = 600)
      ...
      tryCatch({
        if (input$radio == 2) {
          if (!is.null(cos_mat_reactive_rake()) && 
            ...
            write.csv(cos_mat_reactive_rake(), f1_rake,
          ...
          if (!is.null(cos_mat_reactive_tf()) && 
            ...
            write.csv(cos_mat_reactive_tf(), f1_tf, 
                      row.names = FALSE, col.names = FALSE)
            png(f2_tf, width = 800, height = 600)
  ...
  observeEvent(input$analyze1, {
    if (input$radio == 1) 
    {
      files_preprocessed_data_frequency[["df_1"]] <-
        AnalyzeAndRenderFrequency(input[["file1"]], "barPlot1", "wordTable1", "wordcloud1")
    }
    else if (input$radio == 2)
    {
      files_preprocessed_data_rake[["df_1"]] <- 
        AnalyzeAndRenderRake(input[["file1"]], "barPlot1", "wordTable1", "wordcloud1")
    }
  ...
  observeEvent(input$analyze2, {
    ...
    {
      files_preprocessed_data_frequency[["df_2"]] <- 
        AnalyzeAndRenderFrequency(input[["file2"]], "barPlot2", "wordTable2", "wordcloud2")
    }
    else if (input$radio == 2)
    {
      files_preprocessed_data_rake[["df_2"]] <- 
        AnalyzeAndRenderRake(input[["file2"]], "barPlot2", "wordTable2", "wordcloud2")
    }
  })
  observeEvent(input$analyze3, {
    ...
    {
      files_preprocessed_data_frequency[["df_3"]] <- 
        AnalyzeAndRenderFrequency(input[["file3"]], "barPlot3", "wordTable3", "wordcloud3")
    }
    if (input$radio == 2)
    {
      files_preprocessed_data_rake[["df_3"]] <-
        AnalyzeAndRenderRake(input[["file3"]], "barPlot3", "wordTable3", "wordcloud3")
    }
  })
  observeEvent(input$analyze4, {
    ...
    {
      files_preprocessed_data_frequency[["df_4"]] <- 
        AnalyzeAndRenderFrequency(input[["file4"]], "barPlot4", "wordTable4", "wordcloud4")
    }
    if (input$radio == 2)
    {
      files_preprocessed_data_rake[["df_4"]] <- 
        AnalyzeAndRenderRake(input[["file4"]], "barPlot4", "wordTable4", "wordcloud4")
    }
  })
  observeEvent(input$analyze5, {
    ...
    {
      files_preprocessed_data_frequency[["df_5"]] <- 
        AnalyzeAndRenderFrequency(input[["file5"]], "barPlot5", "wordTable5", "wordcloud5")
    }
    if (input$radio == 2)
    {
      files_preprocessed_data_rake[["df_5"]] <- 
        AnalyzeAndRenderRake(input[["file5"]], "barPlot5", "wordTable5", "wordcloud5")
    }
  })
