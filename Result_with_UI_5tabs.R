...
server <- function(input, output, session) {
  ...
  wordclouds_tf <- reactiveValues()
  wordclouds_rake <- reactiveValues()
  barplot_tf <- reactiveValues()
  barplot_rake <- reactiveValues()
  word_table_tf <- reactiveValues()
  word_table_rake <- reactiveValues()
  ...
  AnalyzeAndRenderRake <-  function(file_input, id_plot_output, id_table_output, id_wordcloud_output) { 
    ...
    output[[id_plot_output]] <- renderPlot({
      ...
      # # wordclouds_rake[[id_wordcloud_output]] <- wordcloud_rake
    ...
    output[[id_wordcloud_output]] <- renderWordcloud2({
      ...
      Wordcloud2a(head(keywords_rake_df_for_output_wordcloud, amount_of_words_in_wordcloud), size = 0.45)
    })
  ...
  AnalyzeAndRenderFrequency <- function(file_input, id_plot_output, id_table_output, id_wordcloud_output) {   
    ...
    output[[id_plot_output]]  <- renderPlot({
      ...
      name_of_input_file <- file_input$name
      ggsave(paste(name_of_input_file, "TF Barplot.png"), plot = barplot_tf, width = 8, height = 6, dpi = 300)
      barplot_tf
    })
  ...
  output$downloadData1 <- downloadHandler(
    ...
    content = function(file) {
      ...
      # 
      # name_of_file_wordcloud_rake <- 
      # View(file)
      # ggsave("Облако слов RAKE.png", plot = wordclouds_rake[['wordcloud1']],
      #        width = 8, height = 6, dpi = 300,  device = "png")
      
      data <- 'dsf'
      write.csv(data, file)
      # 
      # # 1. Сохраняем облако слов в PNG
      # png_file1 <- "wordcloud.png"
      # png(png_file1, width = 1000, height = 800)
      # print(wordcloud1())
      # dev.off()
      # 
      # # 2. Сохраняем столбчатую диаграмму в PNG
      # png_file2 <- "barplot.png"
      # png(png_file2, width = 800, height = 600)
      # print(barPlot1())
      # dev.off()
      # 
      # # 3. Сохраняем таблицу в CSV (с кодировкой UTF-8)
      # csv_file <- "word_table.csv"
      # write.csv(wordTable1(), csv_file, row.names = FALSE, fileEncoding = "UTF-8")
      # 
      # # Список файлов для архива
      # files_to_zip <- c(png_file1, png_file2, csv_file)
      # 
      # # Создаем ZIP-архив
      # zip_file <- paste0("results_", Sys.Date(), ".zip")
